...
const MapSkeleton = () => {
  ...
  return (
    <div className="h-[460px] sm:h-[540px] md:h-[620px] rounded-3xl bg-slate-100 border border-slate-200 flex flex-col items-center justify-center animate-pulse">
      ...
      <span className="text-xs font-semibold text-slate-500">{translations.mapLoading || 'Xarita yuklanmoqda...'}</span>
    </div>
  );
};
...
const HomePage: React.FC = () => {
  ...
  const handleOpenOwnerModal = () => {
    if (!isLoggedIn) {
      showToast(translations.loginToContinue || "Maydoningizni qo‘shish uchun avval tizimga kiring yoki ro‘yxatdan o‘ting", "info");
  ...
  const quickTags = [
    ...
    { label: '📍 ' + (translations.yunusabad || 'Yunusobod'), active: searchFilters.district === 'yunusabad', action: () => setSearchFilters({ district: searchFilters.district === 'yunusabad' ? '' : 'yunusabad' }) },
    { label: '📍 ' + (translations.chilanzar || 'Chilonzor'), active: searchFilters.district === 'chilanzar', action: () => setSearchFilters({ district: searchFilters.district === 'chilanzar' ? '' : 'chilanzar' }) },
  ];
            ...
            <>
              ...
              <div className="mb-6 rounded-3xl border border-slate-200/80 bg-white p-6 sm:p-8 shadow-soft text-center">
                <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight mb-2">
                  {translations.heroTitle || 'Futbol maydonlarini qidirish va band qilish'}
                </h1>
                <p className="text-slate-500 text-sm mb-6 max-w-xl mx-auto">
                  {translations.heroSubtitle || 'Toshkentdagi eng yaxshi mini-futbol maydonlari, narxlar va qulay vaqtlarni toping'}
                </p>
