...
6.  **Taqqoslash:** Backend'da hisoblangan `Calculated Hash` va Telegram yuborgan `hash` qiymatlari o'zaro solishtiriladi. Agar ular bir xil bo'lsa, ma'lumotlar haqiqatdan ham Telegram serveridan kelgan va yo'lda o'zgartirilmagan deb topiladi.
...
*   Bu usul ham BotFather orqali bot sozlamalaridagi **"Attachment Menu"** bo'limida yoqiladi va veb-sayt havolasi kiritiladi.
...
*   Foydalanuvchi yuborgan fayl nomi zararli belgilarni o'zichiga olishi mumkin (masalan, `../../etc/passwd` kabi yo'llar orqali Path Traversal hujumi).
...
*   Yuklangan rasmlar tarkibida maxfiy ma'lumotlar, jumladan GPS koordinatalari, kamera modeli va yaratilgan vaqti (EXIF ma'lumotlar) bo'lishi mumkin.
...
2.  **Inline tugmalar:** Ushbu xabar tagida ikkita inline tugma bo'ladi: `[✅ Tasdiqlash]` va `[❌ Bekor qilish]`.
...
*   **Foydalanuvchini ogohlantirish:** `bot_notify.py` orqali mijoznining shaxsiy Telegram chatiga xabar yuboriladi:
...
4.  **Admin panel (Flask):** Tizim ma'murlari va maydon egaler uchun buyurtmalar oqimi va foydalanuvchilar ma'lumotlarini boshqarish paneli.
